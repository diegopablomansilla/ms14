...
public class AsientoModeloService {
	...
	public Integer count(AsientoModeloFilterQ1 filter) throws Exception500 {		
	...
	public AsientoModelo findById(String id) throws Exception500 {
		try {
		...
		AsientoModeloFilterQ1 filter = new AsientoModeloFilterQ1();
		filter.setId(id);

		List<AsientoModelo> items = find(filter);
	...
	public boolean deleteById(String id) throws Exception500 {		
		...
		try {
			...
			boolean b = dataBase.deleteObjectById(id, AsientoModelo.class);
			if (b == false) {
				throw new IllegalStateException(
						"No se pudo borrar el " + AsientoModelo.class.getCanonicalName() + " " + id);
			}
	...
	public AsientoModelo update(AsientoModelo obj) throws Exception500 {
		...
		try {
			...
			if (b == false) {
				throw new IllegalStateException(
						"No se pudo actualizar el " + AsientoModelo.class.getCanonicalName() + " " + obj.getId());
			}
