...
public class CuentaContableEstadoService {
	...
	public Integer count(CuentaContableEstadoFilterQ1 filter) throws Exception500 {		
	...
	public CuentaContableEstado findById(String id) throws Exception500 {
		try {
		...
		CuentaContableEstadoFilterQ1 filter = new CuentaContableEstadoFilterQ1();
		filter.setId(id);

		List<CuentaContableEstado> items = find(filter);
	...
	public boolean deleteById(String id) throws Exception500 {		
		...
		try {
			...
			boolean b = dataBase.deleteObjectById(id, CuentaContableEstado.class);
			if (b == false) {
				throw new IllegalStateException(
						"No se pudo borrar el " + CuentaContableEstado.class.getCanonicalName() + " " + id);
			}
	...
	public CuentaContableEstado update(CuentaContableEstado obj) throws Exception500 {
		...
		try {
			...
			if (b == false) {
				throw new IllegalStateException(
						"No se pudo actualizar el " + CuentaContableEstado.class.getCanonicalName() + " " + obj.getId());
			}
